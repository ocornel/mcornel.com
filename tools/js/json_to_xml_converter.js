$(document).ready(function(e) {
	...
	$("#convert").click(function(e) {
		...
		try {
			if(data != "")
			{
				json = json2xml.convert(JSON.parse(data));
				json = '<?xml version="1.0" encoding="UTF-8" ?>\n' + json;
				editorAce2.setValue(vkbeautify.xml(json, 4));
			}
		} catch (e) {
			editorAce2.setValue("Error in json data!");
		}
    });
	...
	$("#minify_xml").click(function(e) {
		...
		if(data != "")
		{
			editorAce2.setValue(vkbeautify.xmlmin(data, false));
		}
    });
	...
	$("#minify_json").click(function(e) {
		...
		{
			try{
				editorAce1.setValue(vkbeautify.jsonmin(data));
			}catch(e){}
		}
    });
