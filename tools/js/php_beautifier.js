$(document).ready(function(e) {
    ...
    editorAce1.getSession().setMode("ace/mode/php");
    ...
    editorAce2.getSession().setMode("ace/mode/php");
	...
	$("#beautify_php").click(function(e) {
        e.preventDefault();
		data = $.trim(editorAce1.getValue());
		if(data != "")
		{
			indentation_style = $.trim($("#indentation_style").val());
			indent_with = $.trim($("#indent_with").val());
			indentation_size = $.trim($("#indentation_size").val());
			ArrayNested = $.trim($("#ArrayNested:checked").val());
			EqualsAlign = $.trim($("#EqualsAlign:checked").val());
			Fluent = $.trim($("#Fluent:checked").val());
			KeepEmptyLines = $.trim($("#KeepEmptyLines:checked").val());
			ListClassFunction = $.trim($("#ListClassFunction:checked").val());
			Lowercase = $.trim($("#Lowercase:checked").val());
			
			editorAce2.setValue("Please wait while beautifying your code.");
			
			$.ajax({
				type	: "POST",
				url		: "/pb.php",
				data	: {data : data, indentation_style : indentation_style, indent_with : indent_with, indentation_size : indentation_size, ArrayNested : ArrayNested, EqualsAlign : EqualsAlign, Fluent : Fluent, KeepEmptyLines : KeepEmptyLines, ListClassFunction : ListClassFunction, Lowercase : Lowercase},
				success : function(data)
				{
